...
beta_ages[1:20, 1:20] <- ages_params["beta_j"] # transmission rate for juveniles 
beta_ages[21:30, 21:30] <- ages_params["beta_a"] # transmission rate for adults
beta_ages[1:20, 21:30] <- ages_params["beta_aj"] # lower transmission rate between juveniles and adults
beta_ages[21:30, 1:20] <- ages_params["beta_aj"] # lower transmission rate between juveniles and adults
```
...
```{r}
# Using a list instead of a vector to hold the parameters, as beta_ages and aging 
# are both matrices, so we want to keep them as matrices, rather than flattening
multistage_params <- list(
    beta_ages = beta_ages,
    gamma_ages = ages_params[4],
    births_ages = ages_params[5],
    aging = aging
)
...
sol_ms_tibble <- tibble(
        time = sol_ms[, 1],
        juvies = apply(infects_ms[, juvies], 1, sum),
        adults = apply(infects_ms[, adults], 1, sum)
    ) %>%
    pivot_longer(
        cols = c(juvies, adults),
        names_to = "age_group",
...
ggplot(seroprev_ms_tibble, aes(x = age, y = seroprev, fill = age)) +
    ...
    scale_fill_continuous(low = "#85C1E9", high = "#154360")
```
...
```{r}
F <- diag(n_r) %*% multistage_params[["beta_ages"]] +
    multistage_params[["aging"]] -
    diag(diag(multistage_params[["aging"]]))
...
```{r}
ggplot(sol_rm, aes(x = time, y = number, color = age, group = age)) +
    geom_line(linewidth = 1.5) +
