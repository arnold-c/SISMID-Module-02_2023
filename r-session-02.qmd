...
beta_ages[1:20, 1:20] <- ages_params["beta_j"]

# transmission rate for adults
beta_ages[21:30, 21:30] <- ages_params["beta_a"]

# lower transmission rate between juveniles and adults
beta_ages[1:20, 21:30] <- ages_params["beta_aj"]

# lower transmission rate between juveniles and adults
beta_ages[21:30, 1:20] <- ages_params["beta_aj"]
```
...
ja_multistage_model <- function (t, x, p, ...) {
    ...
    didt <- lambda * s + aging_mat %*% i - gamma_ages * i
    drdt <- aging_mat %*% r + gamma_ages * i 
...
ms_stable_n <- solve(
    aging_mat,
    c(-1 * multistage_params[["births_ages"]], rep(0, 29))
)
```
...
```{r}
F <- diag(ms_stable_n) %*% multistage_params[["beta_ages"]] +
    multistage_params[["aging_mat"]] -
    diag(diag(multistage_params[["aging_mat"]]))

V <- diag(multistage_params[["gamma_ages"]] - diag(multistage_params[["aging_mat"]]))
...
v_minus <- rep(multistage_params[["gamma_ages"]], 30) +
