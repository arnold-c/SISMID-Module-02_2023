...
f_mat <- multistage_params[["beta_mat"]] * multistage_stable_n

# set diag of non-inf transition jac pde
v_mat <- diag(
    multistage_params[["recovery"]] + diag(multistage_params[["aging_mat"]])
)

# set off-diag of non-inf transition jac pde to negative aging of prev age group
...
# Find the equilibrium values for each infected age group (last time point)
...
```{r}
# Create dataframe of updated age solution for plotting
...
#| column: body
#| out-width: 100%
# Plot number of individuals in each age group and state over time
...
### Interactive Plot Exercises
  
<div class="exercise">
#### Play with the Vaccination coverage and observe what happens to the mean age of infection.
</div>

### R Code

Now let's look at how we can investigate this using R.
Unlike the interactive plot that simply uses @eq-mean-age to calculate the mean age of infection, let's use a more realistic age-structured model.

Let's return to the earlier models with an age-class mixing matrix.
But this time, we'll calculate $R_0$, the mean age of infection, and the number of cases between 15-35 years as we increase the rate of contact.

Recall from the rubella and congenital rubella syndrome (CRS) example that the risk of severe disease outcomes depends on the risk of infection in reproductive age women.
Recall also that increasing vaccination reduces $R_E$ -- so here we'll evaluate at several values of $R_0$ as a proxy for the impact of vaccination.
...
To do so, we'll make a loop and evaluate the code for each of 10 increasing levels of mixing (which will change $R_0$).
...
R0_mean_age_contacts_df <- map_dfr(
    ...
    .f = function(.x) {
        ...
        F_mat <- diag(stable_n) %*% scaled_params[["beta_mat"]] +
            scaled_params[["aging_mat"]] -
            diag(diag(scaled_params[["aging_mat"]]))

        V_mat <- diag(scaled_params[["recovery"]] -
...
R0_mean_age_births_df <- map_dfr(
    ...
    .f = function(.x) {
        ...
        # Calculate R0 with NGM
        F_mat <- diag(stable_n) %*% births_params[["beta_mat"]] +
            births_params[["aging_mat"]] -
            diag(diag(births_params[["aging_mat"]]))

        V_mat <- diag(births_params[["recovery"]] -
