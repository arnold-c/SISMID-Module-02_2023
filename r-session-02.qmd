...
To first investigate the relationship between $R_0$ and the mean age of infection, we will assume that the population is completely susceptible and that the force of infection is constant.
...
where $L$ is the life expectancy $\left(L = \frac{1}{\mu}\right)$ and $R_E$ is the effective reproductive number $\left(R_E = R_0 * (1 - p)$ where $p$ is the fraction of individuals vaccinated).
...
//| echo: false
Re = R0 * (1 - vacc)
Re_str = Re.toLocaleString(undefined, {minimumFractionDigits: 2})
```
...
Re_mean_age_str = Re_mean_age.toLocaleString(undefined, {minimumFractionDigits: 2})
```
...
Plot.plot({
    ...
    marks: [
        Plot.line(mean_age_arrs.R0, {x: "R0", y: "mean_age", stroke: "#1f77b4", strokeWidth: 6}),
        Plot.line(mean_age_arrs.Re, {x: "R0", y: "mean_age", stroke: "#ff7f0e", strokeWidth: 6}),
        R0_mean_age <= 100 ?
            Plot.dot(mean_age_arrs.R0, {x: R0, y: R0_mean_age, r: 12, stroke: "#1f77b4", fill: "#1f77b4"}) :
            null,
        Re_mean_age <= 100 ?
            Plot.dot(mean_age_arrs.Re, {x: R0, y: Re_mean_age, r: 12, stroke: "#ff7f0e", fill: "#ff7f0e"}) :
            null,
        Plot.arrow(mean_age_arrs.Re, {x1: R0, x2: R0, y1: arrow_start, y2: arrow_end, strokeWidth: 4, headLength: 5, inset: 15})
    ]
})
```
