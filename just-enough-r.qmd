...
Looking at the `data` column for just the first row (`cyl == 6`), we see we have a list with one item: the rest of the data that's relevant to the rows where `cyl == 6`
...
However, it also returns a `rowwise` **tibble**, so we would need to use normal `mutate()` syntax (and explicitly return a list column) to get the same effect as before.
