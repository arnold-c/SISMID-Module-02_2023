...
md`${tex`R_0 = ${closed_R0.toLocaleString(undefined, {minimumFractionDigits: 2})}`}`
```
...
md`${tex`R_0 = ${demographic_R0.toLocaleString(undefined, {minimumFractionDigits: 2})}`}`
```
...
Plot.plot({
  ...
  marks: [
    area ?
      ...
      Plot.lineY(sir_demographic_sol_long, {x: "t", y: "Fraction", stroke: "State", strokeWidth: 3})
  ]
})
```
