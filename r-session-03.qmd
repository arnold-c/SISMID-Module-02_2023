...
```{r}
sse_optim_params <- c(beta = -3.2, S_init = 7.3, I_init = -2.6)  #initial guess

fit1 <- optim(sse_optim_params, sse_sir, data = niamey_df, site = "Site_1") #fit
exp(fit1$par)  #back-transform parameters

fit2 <- optim(sse_optim_params, sse_sir, data = niamey_df, site = "Site_2") #fit
exp(fit2$par)  #back-transform parameters

fit3 <- optim(sse_optim_params, sse_sir, data = niamey_df, site = "Site_3") #fit
exp(fit3$par)
```
...
niamey_optims <- niamey_df %>%
    nest(data = -site) %>%
    mutate(
        fit = map(data, ~optim(sse_optim_params, nested_sse_sir, data = .x)),
