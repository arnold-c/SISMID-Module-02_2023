...
```{r}
#| echo: false
ojs_niamey <- rename(niamey_df, "Biweek" = biweek, "Number of Individuals" = cases)
```
...
function simulate(f, t0, y0, dt, tmax) {
    ...
    var biweek = tsim.map(d => d * 365 / 14)
...
<!-- sir_sol_long = sir_sol.fold(aq.not(['t_annual', 'Biweek']), {as: ['State', 'Number of Individuals']}) -->
```
...
Plot.plot({
    ...
    marks: [
        Plot.lineY(
            sir_sol,
            {x: "Biweek", y: "Number of Individuals"}
        ),
        Plot.dot(
...
niamey_preds_labels <- tibble(
    ...
    x_arrow_end = c(8.5, 7.75),
    y_label = c(120, 600),
    y_arrow_just = c(-15, -70),
    y_arrow_end = c(40, 290),
